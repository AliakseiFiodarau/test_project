...
R::setup( 'mysql:host=127.0.0.1;dbname=bycicle_shop',
    'root', '' );
?>
...
<script>
    ...
    function articleViewsInc() {
        <?
            R::exec('UPDATE `articles` SET `views`= `views`+1 WHERE `article_id`=?',
                array($_SESSION['id']));
        ?>
    }
    function reviewViewInc() {
        <?
        R::exec('UPDATE `reviews` SET `views`= `views`+1 WHERE `review_id`=?',
            array($_SESSION['id']));
        ?>
    }
    sign_in_button.addEventListener('click', openSignIn);
    close_button.addEventListener('click', closeSignIn);
    article_link.addEventListener('click', articleViewsInc);
    review_link.addEventListener('click', reviewViewInc);
</script>
</body>
</html>